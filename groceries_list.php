...
if(isset($_POST['submit_groceries_list'])){
    ...
    $num = htmlspecialchars($_POST['number_pple']);
    $instructions = '';
    $cook_time = 0;

    $stmt = $db->prepare("SELECT link.ingredient_id, link.recipe_id, link.quantity, link.measurement, recipes.recipe_name, recipes.instructions, ingredients.ingredient_name, recipes.cook_time FROM link LEFT JOIN ingredients ON link.ingredient_id=ingredients.ingredient_id LEFT JOIN recipes ON link.recipe_id=recipes.recipe_id WHERE link.recipe_id=? ");
